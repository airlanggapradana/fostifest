...
import {useGetCompeById, usePayment, useRegisterIndividual, useRegisterTeam} from "@/utils/query.ts";
...
const RegisterCompetition = () => {
  ...
  const {mutateAsync: registerIndividual} = useRegisterIndividual();
  const {mutateAsync: registerTeam} = useRegisterTeam();
  const {mutateAsync: handlePayment} = usePayment()

  const onSubmit = async (values: FormValues) => {
    setSubmitError(null);
    try {
      if (teamMode) {
        const payload = values as RegistrationTeamSchema;
        const reg = await registerTeam({
          competitionId: payload.competitionId,
          leaderId: payload.leaderId,
          teamName: payload.teamName,
          memberNames: payload.memberNames,
          memberEmails: payload.memberEmails,
          memberPhoneNumbers: payload.memberPhoneNumbers,
        });
        ...
        const reg = await registerIndividual({
          competitionId: payload.competitionId,
      ...
      const message =
        e instanceof Error ? e.message : "Failed to submit registration. Please try again.";
